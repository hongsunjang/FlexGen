...
from flexgen._dist_utils import initialize_distributed
...
class DistOptLM(OptLM):
    ...
    def compute_layer(self, t, i, j, k):
        ...
        self.layers[j].forward(
                self.hidden[t][i][j][k], 
                self.cache_read_buf[t][j][k],
                self.weight_read_buf[j], 
                self.attention_mask[t][k],
    ...
    def update_attention_mask(self, b, t, i, k):
        if i == 0 : # Prefill!!
            ...
            #print("Left: ", left, ",Right::", right, "prompt_len", self.task.prompt_len)
            ...
            val = attention_compute.allocate(
                (self.policy.gpu_batch_size, self.task.prompt_len), 
                bool
            )
            #print("input ids: ", input_ids)
            val.load_from_np((input_ids != self.config.pad_token_id))
            #print("Padded Token:", self.config.pad_token_id)
            self.attention_mask[t][k].val = val

            mask = self.attention_mask[t][k]
            #print("Prefill Mask shape: ", mask.val.shape)
            #print("Prefill Mask size: ", sum([ int(a) for a in list(mask.val.data[0]) ]) )

        else: # Decode
            #raise NotImplementedError
            mask = self.attention_mask[t][k]
            #print("Decode Mask shape: ", mask.val.shape)
            #print("Decode Mask size: ", sum([ int(a) for a in list(mask.val.data[0]) ]))
...
def run_flexgen_dist(args):
    ...
    model = DistOptLM(opt_config, env, args.path, policy, 
                      #args.rank, #pipeline rank
                      (args.rank % 2), #pipeline rank
                      #args.world_size, # pipeline stages
                      2, # pipeline stages
                      args.comm_device, 
                      num_inner_iterations=num_inner_iterations, # pipeline stages
...
if __name__ == "__main__":
    ...
    if args.head_ip is not None and args.port is not None:
        ...
        d = initialize_distributed(args.head_ip, args.port, args.world_size,
